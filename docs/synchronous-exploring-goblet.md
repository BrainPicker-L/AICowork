...
const SETTINGS_SECTIONS = [
  ...
  { id: 'output', label: 'Output Styles', icon: Palette },
  { id: 'recovery', label: '会话恢复', icon: History },
];
...
### 3.5 官方 Claude Code CLI 机制分析

#### 3.5.1 官方配置结构

**settings.json 完整结构**（参考 `C:\Users\Alan\.claude\settings.json`）：
...
#### 3.5.2 Skills vs Plugins vs MCP 对比

| 类型 | 位置 | 调用方式 | 实现机制 |
|------|------|----------|----------|
| **Skills** | `~/.claude/skills/` | `/skill-name` 或自然语言 | AI 读取 SKILL.md 并按指导执行 |
| **Plugins** | `~/.claude/plugins/cache/` | `/plugin:command` | SDK 直接调用（精确命令） |
| **MCP** | settings.json 配置 | 自动启动 | SDK 启动服务器并注册工具 |
...
**问题 1：所有命令转换为自然语言**
```typescript
// 当前错误实现（slash-commands.ts）
return `请使用 ${commandName} 技能${argsText}`;
```

**问题 2：插件命令未被识别**
- `/pr-review-toolkit:review-pr` 被转换为"请使用 pr-review-toolkit:review-pr 技能"
- SDK 无法识别这是一个插件命令

**问题 3：MCP 服务器未启动**
- 虽然读取了 MCP 配置，但没有传递给 SDK
- SDK 无法启动 MCP 服务器
...
// ❌ 错误方式：通过环境变量传递 MCP 配置
// const env = { MCP_SERVERS: JSON.stringify(mcpServers) };

// ✅ 正确方式：SDK 自动从 ~/.claude/settings.json 读取
// 我们只需要确保环境变量正确设置，SDK 会自动：
// 1. 读取 ~/.claude/settings.json 中的 mcpServers
// 2. 启动配置的 MCP 服务器
// 3. 将 MCP 工具注册到会话中
...
```typescript
// src/electron/libs/slash-commands.ts
...
export function transformSlashCommand(userInput: string): string {
  ...
  if (commandName.includes(':')) {
    return userInput;  // 直接返回原始命令
  }

  // ✅ MCP 服务器命令：直接传递（不转换）
  const mcpServers = await fetchMcpServers();
  if (mcpServers.some(cmd => cmd.name === `/${commandName}`)) {
    return userInput;
  }
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="text-sm text-muted">
          配置 Model Context Protocol 服务器，SDK 会自动启动并注册工具
        </p>
      </header>
      ...
      {/* 添加服务器按钮 */}
      <button className="w-full py-3 rounded-xl border-2 border-dashed border-ink-900/10 text-sm text-muted hover:border-accent/50">
        + 添加 MCP 服务器
      </button>
...
**问题**：当前所有命令都被转换为自然语言，插件命令无法工作。

**修复内容**：

1. **修改 slash-commands.ts**
   ```typescript
   // src/electron/libs/slash-commands.ts
...
2. **测试验证**
   - 测试插件命令：`/pr-review-toolkit:review-pr`
   - 测试 MCP 命令：`/github`（如果已配置）
   - 测试技能命令：`/git-commit`
...
**注意**：SDK 会自动从 `~/.claude/settings.json` 读取 MCP 配置，我们只需要提供设置界面。

1. **后端实现**
   - 创建 `mcp-store.ts` - 读写 settings.json 的 mcpServers
   - 添加 IPC 处理器：
     - `getMcpServers()` - 获取 MCP 服务器列表
     - `saveMcpServer(server)` - 保存单个服务器配置
     - `deleteMcpServer(name)` - 删除服务器配置
     - `testMcpConnection(server)` - 测试 MCP 连接

2. **前端实现**
   - McpSection.tsx - MCP 设置界面
   - McpServerCard.tsx - 服务器卡片组件
   - McpServerDialog.tsx - 添加/编辑对话框
   - 扩展 electron.d.ts - 添加类型定义

3. **runner.ts 修正**
   - 移除任何尝试通过环境变量传递 MCP 配置的代码
   - SDK 会自动读取 ~/.claude/settings.json

### 阶段四：其他区域（优先级：中）

1. **创建框架组件**
   - HelpSection, FeedbackSection, AboutSection
   - SkillsSection, PluginsSection, MemorySection
...
| 文件 | 修改类型 | 说明 |
|------|---------|------|
| `src/electron/libs/slash-commands.ts` | **修正** | 插件/MCP 命令直接传递 |
| `src/ui/store/useAppStore.ts` | 扩展 | 添加页面状态 |
| `src/ui/components/App.tsx` | 修改 | 添加页面切换 |
| `src/ui/components/Sidebar.tsx` | 修改 | 设置按钮行为 |
| `src/ui/components/SettingsModal.tsx` | 迁移 | 提取逻辑到 ApiSection |
| `src/electron/libs/mcp-store.ts` | 新增 | MCP 配置读写 |
| `src/electron/libs/runner.ts` | 修正 | 移除错误的 MCP 环境变量 |
| `src/electron/handler.ts` | 扩展 | MCP IPC 处理器 |
| `src/ui/electron.d.ts` | 扩展 | MCP IPC 接口 |
...
**建议优先级**：
1. **阶段零（立即修复）**：修复斜杠命令系统，让插件和 MCP 命令正常工作
2. **阶段一**：基础架构 + API 设置迁移
3. **阶段三**：MCP 功能完整实现
4. 其他区域可后续迭代

**关键发现**：
- SDK 会自动从 `~/.claude/settings.json` 读取 MCP、插件、钩子配置
- 我们不需要手动传递这些配置，只需提供设置界面
