...
DtazziCowork 使用 `dingtalk-stream` SDK 通过 WebSocket 长连接接收消息，这要求机器人必须以**企业内部应用**的形式创建，并在应用内添加机器人能力。如果通过独立"机器人"入口创建，即使拿到了 AppKey 和 AppSecret，也无法建立 Stream 连接，表现为**连接成功但永远收不到消息**。
...
2. 进入 **应用开发** → **企业内部开发**
3. 点击 **创建应用**，填写应用名称（如"DtazziCowork 助手"）和描述
4. 创建完成后进入应用详情页
...
这两个值需要填入 DtazziCowork 设置页的钉钉机器人配置中。
...
### 第六步：在 DtazziCowork 中配置

1. 打开 DtazziCowork → 设置 → 钉钉机器人
2. 点击 **添加机器人**
3. 填写：
   - **机器人名称**：任意名称，用于在 DtazziCowork 中识别
...
2. 查看 DtazziCowork 日志中是否有 `[dingtalk:xxx] Reply send failed` 相关错误
...
2. 如果 DtazziCowork 中群聊策略设置为"白名单"，确认已添加对应的群会话 ID

### 问题 5：应用重启后钉钉会话丢失

**说明**：这不是 bug。DtazziCowork 会自动从数据库恢复钉钉会话映射，同一个 (用户, 机器人) 组合始终保持同一个会话。如果配置中启用了机器人，应用启动时会自动重新连接。
